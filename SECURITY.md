...
Pipeline diagnostics store only timings, counts, safe request identifiers, exception class/stage and existing redacted error locations. No prompts, credentials, user/session identifiers or raw database exceptions. X-GCA-Request-ID is strictly format-validated and confers no privileges. Metadata is shown only in the existing administrator settings diagnostics.
...
## 13. Provider & Model Selection Security Policy (Node N21)
- **Safe Metadata Exposure:** The `/wp-json/gca/v1/providers` endpoint and localized `gcaConfig.providerSelection` expose strictly non-sensitive metadata: provider `id`, display `name`, `default_model`, and supported `models` list. API keys, internal endpoint URLs, authorization headers, and secret constants are completely omitted.
- **Enabled & Configured Guard:** Public metadata only includes providers that are both explicitly enabled (`is_enabled() === true`) and configured with a valid API key (`is_configured() === true`). Disabled or unconfigured providers are withheld from client discovery.
- **Server-Side Authorization & Strict Override:** Public selection toggles (`allow_public_provider_selection` and `allow_public_model_selection`) dictate whether visitor choices are honored. When toggles are disabled, visitor selections are silently overridden by administrative defaults, preventing forced model exploitation.
- **Cross-Provider Model Injection Prevention:** Every requested model is strictly validated against the target provider via `ModelRegistry::has_model($provider, $model)`. An OpenAI model identifier sent with a Claude or Gemini provider request is rejected with `400 Bad Request` (`INVALID_MODEL`).
- **Mid-Conversation State Safety:** Mid-conversation switching safely isolates provider-specific interaction states (such as Gemini's ephemeral interaction ID) without corrupting conversation history in the database.
