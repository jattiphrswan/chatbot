...
Normal-chat pipeline update: Gemini normal and explicit-test generation both use 45s; gemini-3.8-flash defaults to low thinking. Recent context now queries newest 20 messages and reverses them into chronological order, then caps history at 12,000 characters. Greetings bypass RAG/handoff. RAG respects existing top-K (1?8) and character settings (500?12,000), including the first chunk; framing is bounded to the configured body budget plus 2,000 characters. No WooCommerce/product/email work or analytics logging occurs in this ordinary ChatService path except explicit handoff intent. Safe stage timers, checkpoints, character counts and PHP max_execution_time are recorded per latest request. Post-processing begins after generation, separate from initial user persistence. Proxy timeouts cannot be established from PHP settings alone.

Timeout investigation supersedes the earlier retry patch: generation sends once. Admin Test Connection uses authenticated GET /v1beta/models (20s, TLS verification), stops on failure, then POST generateContent (45s, Reply with OK.). Normal chat remains 30s. gemini-3.8-flash defaults to generationConfig.thinkingConfig.thinkingLevel=low; explicit generation_config remains supported. Separate safe models/generation timings, WP_Error evidence and failure layer are stored and summarized in the existing admin Last Error field. Timeout alone remains NETWORK, not proof of generation latency or hosting firewall.

Gemini generation retries explicit HTTP 500/502/503/504 failures at most twice with 1s then 2s delays, sharing the original timeout budget. Credentials, quota, transport and response-validation failures are not retried. The selected model and payload stay unchanged. Final server-side generation diagnostics include attempt_count.
