...
class RateLimiter {
	...
	public const DEFAULT_5M_LIMIT      = 10;
	public const DEFAULT_1H_LIMIT      = 50;
	 ...
	 */
	public function check_and_consume( string $session_id, ?string $client_ip = null ) {
		...
		// 1. Resolve limits.
		$limit_5m    = absint( $this->settings_service->get( 'rate_limit_5m', self::DEFAULT_5M_LIMIT ) );
		$limit_1h    = absint( $this->settings_service->get( 'rate_limit_1h', self::DEFAULT_1H_LIMIT ) );
		$limit_5m    = $limit_5m > 0 ? $limit_5m : self::DEFAULT_5M_LIMIT;
		$limit_1h    = $limit_1h > 0 ? $limit_1h : self::DEFAULT_1H_LIMIT;
		$ip_limit_5m = $limit_5m * self::IP_CEILING_MULTIPLIER;
		...
		// 3. Check all 4 rate limit windows.
		$check_s5  = $this->inspect_window( $key_s5, $limit_5m );
		$check_s60 = $this->inspect_window( $key_s60, $limit_1h );
		$check_i5  = $this->inspect_window( $key_i5, $ip_limit_5m );
		...
		if ( $retry_after > 0 ) {
			return new WP_Error(
				'RATE_LIMITED',
				__( 'Too many requests. Please wait a moment before sending another message.', 'gemini-chat-assistant' ),
				[
					'status'      => 429,
		...
		// 4. Consume budget by incrementing all counters.
